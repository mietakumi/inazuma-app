...
### 主要コンポーネント
- **フォーカス計算** ([app/calc/page.tsx](app/calc/page.tsx)): ステータス・装備・バフを組み合わせてAT/DF値を算出
- **キャラクター図鑑** ([app/character/page.tsx](app/character/page.tsx)): ポジション・ビルド別フィルタ対応の一覧表示
- **投稿システム** ([app/submit/page.tsx](app/submit/page.tsx)): ユーザーによるキャラクター情報投稿（重複チェック付き）
- **管理画面** ([app/admin/page.tsx](app/admin/page.tsx)): パスキー認証で投稿承認/却下を実施
...
### データベース（[db/schema.sql](db/schema.sql)）
...
- 型定義は [types/index.ts](types/index.ts) に集約
...
### レスポンシブ対応
[app/layout.tsx](app/layout.tsx) の `<style>` タグで全体レイアウトを制御:
```css
...
### 型安全性
- 全ての型は [types/index.ts](types/index.ts) から `import` して使用
...
npm run lint
```
`eslint-config-next` を使用（[eslint.config.mjs](eslint.config.mjs)）
...
- グローバルスタイルは [app/globals.css](app/globals.css) で管理
...
3. Supabaseテーブル変更時は [db/schema.sql](db/schema.sql) を更新
4. [app/layout.tsx](app/layout.tsx) のサイドバーメニューにリンク追加
5. [app/page.tsx](app/page.tsx) のお知らせ・ツール一覧に記載
